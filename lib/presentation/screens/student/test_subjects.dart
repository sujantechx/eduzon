// lib/presentation/screens/student/test_subjects_screen.dart
...
class _TestSubjectsState extends State<TestSubjects> {
  ...
  void _openTestChapters(SubjectModel subject) {
    final authState = context.read<AuthCubit>().state;
    if (authState is Authenticated) {
      final user = authState.userModel;
      context.push(AppRoutes.testChapter, extra: {'subject': subject, 'courseId': user.courseId});
    }
  }

  Widget _listTile(SubjectModel s) {
    return ListTile(
      onTap: () => _openTestChapters(s),
      contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
      leading: CircleAvatar(
        radius: 26,
        backgroundColor: Colors.deepPurple.shade50,
        child: Text(s.subjectNumber != null ? '${s.subjectNumber}' : '?', style: const TextStyle(color: Colors.deepPurple, fontWeight: FontWeight.bold)),
      ),
      ...
      trailing: const Icon(Icons.quiz, color: Colors.deepPurple),
    );
  }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Align(alignment: Alignment.bottomRight, child: Icon(Icons.quiz, color: Colors.deepPurple))
            ],
          ),
        ),
      ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Test Subjects'),
        actions: [
